...
const ExportButtons: React.FC<ExportButtonsProps> = ({ 
  ...
  const getDetailHeader = (filterType: string): string => {
    switch(filterType) {
      ...
      case 'verspaetungen_offen':
        return 'Noch zu entschuldigende Verspätungen';
  ...
  const getStatusColor = (status: string, datum: Date | string): number[] => {
    ...
    if (!status || status.trim() === '') {
      ...
      return today > deadlineDate ? [255, 0, 0] : [255, 215, 0]; // Red if overdue, Yellow if open
    }
  ...
  const formatDetails = (details: AbsenceEntry[]): string[] => {
    return details.map((entry, index) => {
      ...
      return `${index + 1}. ${date}${time}: ${type}${reason}${status}`;
    });
  };
    ...
    if (isReportView) {
      return data.map(([student, stats], index) => {
        ...
        return {
          ...
          'Vorname': vorname,
          'Klasse': stats.klasse,
          'Unentschuldigte Verspätungen': formattedLates.length > 0 ? formattedLates : ['-'],
          'Unentschuldigte Fehlzeiten': formattedAbsences.length > 0 ? formattedAbsences : ['-'],
  ...
  const exportToPDF = () => {
    ...
    doc.text(`Zeitraum: ${startDateFormatted} - ${endDateFormatted}`, margin.left, margin.top + 7);

    if (!isReportView) {
      doc.text(`Wochen für Berechnung: ${selectedWeeks}`, margin.left, margin.top + 14);
      doc.setFont('helvetica', 'italic');
      doc.text('Legende: (E) = Entschuldigt, (U) = Unentschuldigt, (O) = Offen, SJ = Schuljahr, (V) = Verspätungen, (F) = Fehlzeiten', 
              margin.left, margin.top + 21);
    }
    ...
    const calculateColumnWidths = () => {
      const baseWidths = isReportView ? 
        [10, 30, 30, 15, 80, 80] : 
        [25, 25, 15, 15, 15, 15, 15, 15, 15, 20, 20, 25, 25];
    ...
    const columnStyles = hasDetails ? {
      ...baseColumnStyles,
      'Details': { cellWidth: 0, minCellWidth: 80 }
    } : baseColumnStyles;
    ...
    const tableData = {
      ...
      body: enrichedData.flatMap(row => {
        ...
        delete displayRow['_lateEntries'];
        delete displayRow['_absenceEntries'];

        if (isReportView) {
          const lateEntries = row['_lateEntries'] || [];
          const absenceEntries = row['_absenceEntries'] || [];
          
          const lateRows = lateEntries.map((entry: AbsenceEntry, idx: number) => ({
            content: `${idx + 1}. ${formatDate(entry.datum)} (${entry.beginnZeit} - ${entry.endZeit} Uhr)${entry.grund ? ` - ${entry.grund}` : ''}${entry.status ? ` [${entry.status}]` : ''}`,
            styles: { textColor: getStatusColor(entry.status || '', entry.datum) }
          }));
          
          const absenceRows = absenceEntries.map((entry: AbsenceEntry, idx: number) => ({
            content: `${idx + 1}. ${formatDate(entry.datum)} - ${entry.art}${entry.grund ? ` (${entry.grund})` : ''}${entry.status ? ` [${entry.status}]` : ''}`,
            styles: { textColor: getStatusColor(entry.status || '', entry.datum) }
          }));
          ...
          const mainRow = [
            ...
            displayRow['Vorname'],
            displayRow['Klasse'],
            lateRows.length > 0 ? lateRows[0].content : '-',
            absenceRows.length > 0 ? absenceRows[0].content : '-'
          ];

          const additionalRows = [];
          const maxDetails = Math.max(lateRows.length, absenceRows.length);
          for (let i = 1; i < maxDetails; i++) {
            additionalRows.push([
              '', '', '', '',
              i < lateRows.length ? lateRows[i].content : '',
              i < absenceRows.length ? absenceRows[i].content : ''
            ]);
          }
    ...
    autoTable(doc, {
      ...
      didParseCell: (data) => {
        if (data.section === 'body' && isReportView && data.column.index >= 4) {
          const rowData = enrichedData[data.row.index % enrichedData.length];
          const entries = data.column.index === 4 ? rowData['_lateEntries'] : rowData['_absenceEntries'];
          if (entries && data.row.cells[data.column.index]?.raw) {
            const idx = data.row.cells[data.column.index].raw.split('.')[0];
            const entryIndex = parseInt(idx) - 1;
            if (entryIndex >= 0 && entryIndex < entries.length) {
              const entry = entries[entryIndex];
              data.cell.styles.textColor = getStatusColor(entry.status || '', entry.datum);
            }
          }
        }
      }
